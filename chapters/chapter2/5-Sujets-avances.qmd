...
-  `scikit-learn` propose également une approche similaire à celle d'XGBoost, à une différence près: lorsqu'il n'y a pas de valeurs manquantes au moment de construire la règle de décision, les valeurs manquantes rencontrées en prédiction seront envoyées au noeud-enfant comprenant le plus d'observations^[Voir la [documentation](https://scikit-learn.org/stable/modules/generated/sklearn.ensemble.RandomForestClassifier.html)].
