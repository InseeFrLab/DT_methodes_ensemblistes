...
A l'origine, les arbres CART ont été conçus pour partitionner l'espace en utilisant exclusivement des variables numériques, et ne pouvaient pas mobiliser les variables catégorielles sans un retraitement préalable. Plusieurs techniques d'encodage (_one-hot-encoding_, _ordinal encoding_, _target encoding_) ont donc été développées pour surmonter ce problème en transformant les variables catégorielles en variables numériques. Ces techniques d'encodage ne sont d'ailleurs pas spécifiques aux méthodes ensemblistes à base d'arbres. Depuis le milieu des années 2010, une nouvelle approche efficace et spécifique aux méthodes ensemblistes a été introduite dans les implémentations du _gradient boosting_. Ce paragraphe présente et compare ces différentes approches des variables catégorielles.

Les trois principales approches d'encodage sont les suivantes:
...
La dernière approche, appelée __support natif des variables catégorielles__ (**_native support for categorical features_**) n'est pas une méthode d'encodage et est spécifique aux implémentations du _gradient boosting_. Dans cette approche, on essaie de définir le meilleur _split_ directement à partir d'une variable catégorielle en séparant ses modalités en deux sous-ensembles optimaux (exemple: `{'A', 'B', 'C'}` et `{'D', 'E', 'F'}` pour une variable comprenant six modalités). La force de cette approche est qu'elle évite de faire une énumération exhaustive des partitions possibles[^note_partition] en utilisant la méthode optimale proposée par @fisher1958grouping: à chaque _split_, les modalités sont triées en fonction de $-\frac{\sum_{i \in I_j} g_i}{\sum_{i \in I_j} h_i}$ [^note_native], puis le meilleur _split_ est choisi en testant les différentes segmentations possibles de cette liste triée. Par exemple, si à un _split_ donné les modalités sont triées dans l'ordre `ABDFEC`, l'algorithme testera les splits `A|BDFEC`, `AB|DFEC`, `ABD|FEC`, etc. 

Cela revient à faire du _target encoding_.
...
Elle repose sur un résultat théorique @fisher1958grouping
...
[^note_partition] Une telle énumération s'avère impossible en pratique dès que le nombre de modalités s'élève, car il existe $2^(k-1) - 1$ partitions possibles pour une variable à $k$ modalités. 
...
#figure(
  table(
    ...
    [ Approche par histogramme], [#absent()], [#present()], [#present()], [#present()], [#present()], 
    [ _One-hot-encoding_], [#absent()], [#unknown()], [#unknown()], [#unknown()], [#unknown()], 
    [ _Ordinal encoding_], [#present()], [#unknown()], [#unknown()], [#unknown()], [#unknown()], 
    ...
    caption: [_Preprocessing_ des variables dans les implémentations des méthodes ensemblistes],
) <tbl-preprocessing>
```
:::
...
Dans le _package_ `ranger`, le traitement des variables catégorielles (`factors`) est contrôlé par l'argument `respect.unordered.factors`, qui peut prendre trois valeurs: `ignore` (_ordinal encoding_),  `order` (_target encoding_) et `partition` (essayer toutes les combinaisons possibles). L'usage de cet argument est détaillé dans la documentation de la fonction `ranger()`. L'usage de la modalité `partition` n'est pas recommandé.
