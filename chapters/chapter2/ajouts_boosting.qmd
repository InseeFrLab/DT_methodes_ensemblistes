...
Certaines implémentations des méthodes ensemblistes prennent en charge directement certaines des quatre approches présentées ci-dessus; dans les autres cas, il faut préparer les données en amont de l'entraînement et de la prédiction, par exemple en utilisant un `ColumnTransformer` de `scikit-learn` ([`OneHotEncoder()`](https://scikit-learn.org/dev/modules/generated/sklearn.preprocessing.OneHotEncoder.html), [`TargetEncoder()`](https://scikit-learn.org/dev/modules/generated/sklearn.preprocessing.TargetEncoder.html), [`OrdinalEncoder()`](https://scikit-learn.org/dev/modules/generated/sklearn.preprocessing.OrdinalEncoder.html)). Le tableau suivant résume quelles approches sont proposées par chaque implémentations des méthodes ensemblistes.
...
| Approche                                   | `ranger`   | `scikit-learn` | `XGBoost`  | `LightGBM` | `CatBoost`  |
| ------------------------------------------ | ---------- | -------------- | ---------- | ---------- | ----------- |
| _One-hot-encoding_                         | {{< fa circle-xmark >}} | {{< fa circle-xmark >}} | {{< fa circle-check >}} | {{< fa circle-check >}} | {{< fa circle-check >}} |
| _Ordinal encoding_                         | {{< fa circle-check >}} | {{< fa circle-xmark >}} | {{< fa circle-xmark >}} | {{< fa circle-xmark >}} | {{< fa circle-xmark >}} |
| _Target encoding_                          | {{< fa circle-check >}} | {{< fa circle-xmark >}} | {{< fa circle-xmark >}} | {{< fa circle-xmark >}} | {{< fa circle-check >}} |
| Support natif des variables catégorielles  | {{< fa circle-xmark >}} | {{< fa circle-check >}} | {{< fa circle-check >}} | {{< fa circle-check >}} | {{< fa circle-xmark >}} | 

: _Preprocessing_ des variables catégorielles dans les implémentations des méthodes ensemblistes {#tbl-preprocessing-cat tbl-colwidths="[50,10,10,10,10,10]"}
:::
