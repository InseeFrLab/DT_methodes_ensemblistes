...
Certaines implémentations des méthodes ensemblistes prennent en charge directement certaines des quatre approches présentées ci-dessus; dans les autres cas, il faut préparer les données en amont de l'entraînement et de la prédiction. Le tableau suivant
...
- `LightGBM`: cette implémentation utilise par défaut le _one-hot-encoding_ pour les variables catégorielles comprenant 4 modalités ou moins, et le support natif des variables catégorielles pour les autres. Le paramètre `max_cat_to_onehot` permet de contrôler ce seuil. En revanche, `LightGBM` ne prend pas en charge ni l'_ordinal encoding_ ni le _target encoding_.
