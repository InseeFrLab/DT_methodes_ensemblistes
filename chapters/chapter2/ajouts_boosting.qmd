...
La dernière approche, appelée __support natif des variables catégorielles__ (**_native support for categorical features_**) n'est pas un encodage et est une spécificité des implémentations du _gradient boosting_. Dans cette approche, l'objectif est de déterminer le meilleur _split_ directement à partir des modalités d'une variable catégorielle, en les séparant en deux sous-ensembles optimaux (par exemple : {'A', 'B', 'C'} et {'D', 'E', 'F'} pour une variable comportant six modalités). L'avantage principal de cette approche est qu'elle évite de faire
l'énumération exhaustive des partitions possibles[^note_partition], en recourant à la méthode optimale de @fisher1958grouping: à chaque split, les modalités sont triées selon $-\frac{\sum_{i} g_i}{\sum_{i} h_i}$ [^note_native], puis le meilleur split est choisi en testant les différentes divisions possibles de cette liste triée. Par exemple, si à un moment donné les modalités sont triées dans l'ordre ABDFEC, l'algorithme examinera les splits A|BDFEC, AB|DFEC, ABD|FEC, etc. 

Cette approche peut être considérée comme une variante du _target encoding_, avec deux différences notables: l'encodage des modalités se fait à partir du gradient et de la hessienne de la fonction de perte (et non à partir de $y$), et cet encodage a lieu à chaque _split_ et non une seule fois avant l'entraînement.
