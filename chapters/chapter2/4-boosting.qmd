...
$$
L^{(t)} = \underbrace{ \sum_{i=1}^n \mathcal{l}\bigl(y_i,\;\hat{y}_i^{(t-1)} + f_t(x_i)\bigr)}_{\substack{\text{Perte sur les} \\ \text{observations}}} + \underbrace{\Omega\bigl(f_t\bigr)}_{\substack{\text{Terme de} \\ \text{régularisation}}}\,\,\text{avec}\,\,\Omega(f) = \gamma \, T \;+\; \tfrac{1}{2} \, \lambda \, \|w\|^2
$$ {#eq-fct-obj-initial}
