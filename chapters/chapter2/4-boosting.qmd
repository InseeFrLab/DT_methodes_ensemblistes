...
-   Le modèle global est initialisé à partir des données d'entraînement. A ce stade, le modèle prédit en général la moyenne de la variable-cible.
-   A l'issue de cette initialisation, on calcule la dérivée partielle (*gradient*) de la fonction de perte par rapport à la prédiction pour chaque observation de l'ensemble d'entraînement. Cette dérivée partielle est parfois appelée __pseudo-résidu__. Ce gradient indique à la fois dans quelle direction et dans quelle ampleur la prédiction devrait être modifiée afin de réduire la perte.
<!-- : si le gradient d'une observation est positif, il faut modifier à la baisse la prédiction associée à cette observation pour réduire la perte. -->
...
-   On recalcule la dérivée partielle de la fonction de perte par rapport aux nouvelles prédictions du modèle global qui vient d'être mis à jour.
-   Un deuxième arbre est entraîné à prédire l'opposé du gradient de la fonction de perte, puis est ajouté au modèle global.
