...
2.  Pour $m = 1, \dots, M:$
    ...
    (c) Mettre à jour le modèle global: $F_m\left(\mathbf{x_i}\right) = F_{m-1}\left(\mathbf{x_i}\right) + \rho f_m\left(\mathbf{x}_i\right)$ avec $\rho$ le taux d'apprentissage (_learning rate_) dont la raison d'être est présentée dans la section @sec-overfitting-gb. 
