...
La construction de l'arbre se fait selon une approche **gloutonne**, _nœud après nœud_, de façon très similaire à la construction d'un arbre CART: on divise les données d'entraînement en sous-régions de plus en plus petites, tant que cela génère une réduction "suffisante" de la fonction de perte. Cette approche peut être résumée ainsi:
...
2. Pour ce noeud, on essaie de trouver un partitionnement optimal:
    ...
    - Si la meilleure règle de décision apporte un gain suffisamment élevé, on crée deux nouveaux noeuds-enfant.
