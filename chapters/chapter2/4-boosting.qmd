...
::: {#gbm-pseudocode .callout-note}

## Présentation formelle de la _Gradient Boosting Machine_

On dispose d'un jeu de données ${(x_i, y_i)}_{i=1}^n$ avec $x_i \in \mathbb{R}^m$ et une cible $y_i$. On définit une fonction de perte $l$ qui mesure la distance entre la prédiction $\hat{y}$ et la vraie valeur $y$. Elle présente généralement les propriétés suivantes: elle est convexe et dérivable deux fois, et atteint son minimum lorsque $\hat{y} = y$. On veut entraîner un modèle comprenant $m$ arbres:

$$ \hat{y}_{i} =F\left(\mathbf{x}_i\right) = \sum_{k=1}^{K} f_k\left(\mathbf{x}_i\right) $$

Procédure de _boosting_:
1.  Initialiser le modèle avec $F_0\left(\mathbf{x}\right) = f_0\left(\mathbf{x}\right) = y_0$.
2.  Pour $m = 1, \dots, M:$
    ...
    (b) Entraîner le $m$-ième _weak learner_: on cherche l'arbre $f_m$ défini par les paramètres $\mathbf{a_m}$ qui prédit le mieux l'opposé du gradient de la fonction de perte:
