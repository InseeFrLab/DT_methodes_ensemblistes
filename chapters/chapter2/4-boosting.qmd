...
- l'__hyperparamètre de régularisation $\lambda$__ intervient dans l'équation @eq-w-j-optimal et réduit la valeur absolue des poids des feuilles terminales. Cet hyperparamètre contribue à ce que chaque arbre prévoie des valeurs peu élevées. L'intuition est la suivante: une feuille terminale qui contient un poids $w_i$ élevé en valeur absolue contient probablement des observations inhabituelles ou aberrantes (et dont le gradient $g_i$ prend une valeur extrême); il est donc préférable de réduire légèrement ce poids pour ne pas donner trop d'importance à des points aberrants.
- l'__hyperparamètre de régularisation $\gamma$__ intervient dans l'équation @eq-fct-eval-split
