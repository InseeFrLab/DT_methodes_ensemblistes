...
$$ \hat{f}_t = \arg\min_{f} L^{(t)} = \arg\min_{f} \sum_{i=1}^n \ell\left(y_i,\;\hat{y}_i^{(t-1)} + f_t(\mathbf{x}_i)\right) + \sum_k \Omega(f_{k}) $$
