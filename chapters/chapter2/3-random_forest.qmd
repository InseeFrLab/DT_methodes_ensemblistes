...
- **Pertinence pour certains cas d'usage** : les prédictions _out-of-bag_ peuvent être particulièrement utiles lorsqu'on veut utiliser le modèle sur les données d'entraînement. C'est notamment le cas si l'on veut utiliser une forêt aléatoire pour prédire la probabilité de réponse dans une enquête, dans le cadre d'une repondération selon la méthode des groupes de réponse homogène.
